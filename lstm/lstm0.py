...
class LSTMModel(torch.nn.Module):
    """
    A LSTM few to many models
    """

    def __init__(self, network0_params={}, network1_params={'possible_species':[0,1]}, n_in=2, n_out=3, n_target=1):
        super().__init__()
        """
        ...
        self.onehot = OneHotSpecies(species_set=self.hipnn0.species_set)
    ...
    def regularization_params(self):
        params = self.hipnn0.regularization_params() + \
            self.hippn1.regularization_params + \
            self.predict.regularization_params
        for layer in self.W:
            params.append(layer.int_weights)
            params.append(layer.selfint.weight)
        return params
    ...
    def forward(self, species, coordinates, x_raw, n_out=-1):
        """
        ...
        output_features = self.hipnn0(indexed_features, pair_first, pair_second, pair_dist)
...
from hippynn.graphs.nodes.base import MultiNode, AutoKw

class LSTMNode(AutoKw, MultiNode):
    _input_names = "species", "coordinates", "x_raw_t"
    _output_names = "x_output_t", "nonblank"
    _main_output = "x_output_t"
    _child_index_states = IdxType.MolAtom, IdxType.MolAtom
    _auto_module_class = LSTMModel
    
    def __init__(self, name, parents, network0_params={}, network1_params={'possible_species':[0,1]}, n_in=2, n_out=3, module='auto', **kwargs):
        self.module_kwargs = dict(network0_params=network0_params, network1_params=network1_params, n_in=n_in, n_out=n_out)
...
if __name__ == '__main__':
    ...
    output, nonblank = m(Z, R, x)
    y = torch.rand(2,6,3, device=device)
    L = torch.sum((output-y[nonblank])**2)
    L.backward()
    m.eval()
    k,_ = m(Z[0:1], R[0:1], x[0:1], n_out=4)
    print(k)
    #print(L.shape)
    #print(f.shape, Z)
